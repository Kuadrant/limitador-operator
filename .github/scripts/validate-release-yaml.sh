...
for dep in $DEPS; do
  ...
  if [[ "$BRANCH" != "main" && "$dep_version" == "0.0.0" ]]; then
    echo "::error::Dependency '${dep}' is set to '0.0.0' on branch '$BRANCH' -- release branches must pin a concrete dependency version, not latest/main"
    exit 1
  fi
